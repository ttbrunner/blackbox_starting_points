...
def VisualizeImageColor(image_3d, percentile=99):

  image_3d = np.abs(image_3d)

  for channel in range(3):
    vmax = np.percentile(image_3d[..., channel], percentile)
    vmin = np.min(image_3d[..., channel])
    image_3d[..., channel] = np.clip((image_3d[..., channel] - vmin) / (vmax - vmin), 0, 1)
